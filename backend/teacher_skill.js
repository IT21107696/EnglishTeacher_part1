...
app.get("/api/data/teacher_skill/:id?", (req, res) => {
  // here i used sk_id as id
  const tableName = "Teacher_Skill";
  const id = req.params.id;

  // Your SQL query to fetch data from the specified table
  const sqlQuery = id
    ? `SELECT * FROM ${tableName} WHERE sk_id = ?`
...
app.post("/api/data/teacher_skill", (req, res) => {
  const tableName = "Teacher_Skill";
