...
app.get("/api/data/teacher_qualification/:id?", (req, res) => {
  // here i used q_id as id
  const tableName = "Teacher_Qualification";
  const id = req.params.id;

  // Your SQL query to fetch data from the specified table
  const sqlQuery = id
    ? `SELECT * FROM ${tableName} WHERE q_id = ?`
...
app.post("/api/data/teacher_qualification", (req, res) => {
  const tableName = "Teacher_Qualification";
