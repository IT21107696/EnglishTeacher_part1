...
app.get("/api/data/teacher_review/:id?", (req, res) => {
  // here i used re_id as id
  const tableName = "Teacher_Review";
  const id = req.params.id;

  // Your SQL query to fetch data from the specified table
  const sqlQuery = id
    ? `SELECT * FROM ${tableName} WHERE re_id = ?`
...
app.post("/api/data/teacher_review", (req, res) => {
  const tableName = "Teacher_Review";
