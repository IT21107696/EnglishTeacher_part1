...
app.get("/api/data/student/:id?", (req, res) => {
  // here i used stu_id as id
  const tableName = "Student";
  const id = req.params.id;

  // Your SQL query to fetch data from the specified table
  const sqlQuery = id
    ? `SELECT stu_id,stu_fname,stu_lname,phone_num,email,	NIC FROM ${tableName} WHERE stu_id = ?`
    : `SELECT stu_id,stu_fname,stu_lname,phone_num,email,	NIC FROM ${tableName}`;
