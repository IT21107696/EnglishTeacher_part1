...
app.get("/api/data/student_schedule/:id?", (req, res) => {
  // here i used ssch_id as id
  const tableName = "Student_Schedule";
  const id = req.params.id;

  // Your SQL query to fetch data from the specified table
  const sqlQuery = id
    ? `SELECT * FROM ${tableName} WHERE ssch_id = ?`
...
app.post("/api/data/student_schedule", (req, res) => {
  const tableName = "Student_Schedule";
