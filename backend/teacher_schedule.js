...
app.get("/api/data/teacher_schedule/:id?", (req, res) => {
  // here i used tsch_id as id
  const tableName = "Teacher_Schedule";
  const id = req.params.id;

  // Your SQL query to fetch data from the specified table
  const sqlQuery = id
    ? `SELECT * FROM ${tableName} WHERE tsch_id = ?`
...
app.post("/api/data/teacher_schedule", (req, res) => {
  const tableName = "Teacher_Schedule";
